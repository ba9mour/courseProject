...
void MainWindow::onAddTask() {
  ...
  QString title = QInputDialog::getText(
      this, "Новая задача", "Заголовок:", QLineEdit::Normal, "", &ok);
  if (!ok || title.isEmpty())
    return;

  // description
  QString description = QInputDialog::getText(
      this, "Новая задача", "Описание:", QLineEdit::Normal, "", &ok);
  if (!ok)
    return;

  // priority
  int priority = QInputDialog::getInt(this, "Новая задача",
                                      "Приоритет (целое):", 0, 0, 100, 1, &ok);
  if (!ok)
    return;

  // deadline
  QString dueDate = QInputDialog::getText(
      this, "Новая задача", "Дедлайн (YYYY-MM-DD):", QLineEdit::Normal, "",
      &ok);
  if (!ok)
    return;
  ...
  QString statusStr = QInputDialog::getItem(this, "Новая задача",
                                            "Статус:", statuses, 0, false, &ok);
  if (!ok)
    return;
  ...
  // create and add task
  Task t(title.toStdString(), description.toStdString(), priority,
         dueDate.toStdString(), status);
  manager_.addTask(t);
  refreshList();
}
...
void MainWindow::onEditTask() {
  ...
  // edit title
  QString title = QInputDialog::getText(
      this, "Редактировать задачу", "Заголовок:", QLineEdit::Normal,
      QString::fromStdString(old.getTitle()), &ok);
  if (!ok || title.isEmpty())
    return;

  // edit description
  QString description = QInputDialog::getText(
      this, "Редактировать задачу", "Описание:", QLineEdit::Normal,
      QString::fromStdString(old.getDescription()), &ok);
  if (!ok)
    return;

  // edit priority
  int priority =
      QInputDialog::getInt(this, "Редактировать задачу",
                           "Приоритет:", old.getPriority(), 0, 100, 1, &ok);
  if (!ok)
    return;

  // edit deadline
  QString dueDate = QInputDialog::getText(
      this, "Редактировать задачу", "Дедлайн (YYYY-MM-DD):", QLineEdit::Normal,
      QString::fromStdString(old.getDueDate()), &ok);
  if (!ok)
    return;
  ...
  QString statusStr =
      QInputDialog::getItem(this, "Редактировать задачу", "Статус:", statuses,
  ...
  // new task
  Task newT(title.toStdString(), description.toStdString(), priority,
            dueDate.toStdString(), status);
...
void MainWindow::onSaveTasks() {
  ...
  QMessageBox::information(this, "Сохранено",
                           "Задачи сохранены в " +
                               QString::fromStdString(defaultFilename_));
}
